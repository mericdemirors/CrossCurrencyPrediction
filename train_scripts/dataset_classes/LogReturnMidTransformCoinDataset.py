...
class LogReturnMidTransformCoinDataset(Dataset):
    def __init__(self, csv_path, input_coins, input_features, output_coins, output_features, input_window, output_window,
        ...
        cols_to_drop = [col for col in self.df.columns if ("low" in col) or ("high" in col) or ("_open_right" in col) or ("_close_left" in col)]
        self.df = self.df.drop(columns=cols_to_drop)
        
        self.df.columns = self.df.columns.str.replace(r'_(left|right)$', '', regex=True)
