...
class LogReturnTransformPriceLossCoinDataset(Dataset):
    def __init__(self, csv_path, input_coins, input_features, output_coins, output_features, input_window, output_window,
                 price_loss_with_real, price_loss_weight, transform_name, output_distribution, n_quantiles, train_session_dir, training_dataset,
        ...
        self.real_prices_df = pd.read_csv(self.csv_path, index_col="open_time")
        self.real_prices_df = self.real_prices_df[self.df.columns]
        self.price_loss_with_real = price_loss_with_real
    ...
    def __getitem__(self, idx):
        ...
        self.initial_prices_for_price_loss = self.real_prices_df.loc[analysis_rows.index[-1]][self.output_cols].values
        self.prices_to_predict_for_price_loss = torch.from_numpy(self.real_prices_df.loc[prediction_rows.index][self.output_cols].values)
    ...
    def price_loss(self, predictions, target, base_loss_fn):
        batch_predictions_scaled_with_real, batch_predictions_scaled_with_pred = [], []
        for batch_instance in predictions:
            predictions_scaled_with_real, predictions_scaled_with_pred = self.rescale_to_real_price(batch_instance.cpu().detach().T, torch.from_numpy(self.initial_prices_for_price_loss))
            batch_predictions_scaled_with_real.append(predictions_scaled_with_real)
            batch_predictions_scaled_with_pred.append(predictions_scaled_with_pred)
        
        if self.price_loss_with_real:
            return base_loss_fn(predictions_scaled_with_real, self.prices_to_predict_for_price_loss)
        else:
            return base_loss_fn(predictions_scaled_with_pred, self.prices_to_predict_for_price_loss)